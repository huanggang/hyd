...
switch ($method)
{
  ...
  default: // GET
    ...
    $query = "SELECT app_id, app_is_done, app_is_loaned, app_title, app_usr_id, act_info_nick, app_category, app_amount, app_duration, app_status, app_applied, app_comment FROM applications_app LEFT JOIN account_info_act_info ON app_usr_id = act_info_usr_id WHERE app_is_done = ";
    switch ($type)
    {
      case 2: // done
    ...
    while ($row = mysqli_fetch_array($result))
    {
      ...
      $json = $json.",{\"app_id\":".jsonstrval($app_id).",\"is_done\":".jsonstrval($app_is_done).",\"is_loaned\":".jsonstrval($app_is_loaned).",\"title\":".jsonstr($app_title).",\"user_id\":".jsonstrval($app_usr_id).",\"nick\":".jsonstr($act_info_nick).",\"category\":".jsonstrval($app_category).",\"amount\":".jsonstrval($app_amount).",\"duration\":".jsonstrval($app_duration).",\"status\":".jsonstrval($app_status).",\"applied\":".jsonstr($app_applied).",\"comment\":".jsonstr($app_comment)."}";
    }
    mysqli_free_result($result);
