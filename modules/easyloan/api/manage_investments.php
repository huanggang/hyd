...
switch ($type)
{
  ...
  case 2: // published, but not finished yet
    ...
    $start = ($page - 1) * $per_page;
    $query = "SELECT inv_app_id, inv_title, inv_usr_id, act_info_nick, inv_category, lns_amount, lns_interest, lns_interest_rate, lns_repayment_method, lns_duration, lns_start, lns_end, lns_fine_rate, lns_fine_rate_is_single, lns_finished, lns_fine, inv_amount, inv_interest_rate, inv_repayment_method, inv_duration, inv_start, inv_end, inv_investment, inv_interest, inv_fine_rate, inv_fine_rate_is_single, inv_fine, inv_created FROM investments_inv LEFT JOIN account_info_act_info ON inv_usr_id = act_info_usr_id LEFT JOIN loans_lns ON inv_app_id = lns_app_id WHERE inv_is_done=0 ORDER BY inv_created ASC LIMIT ".strval($start).",".strval($per_page);
    $result = mysqli_query($con, $query);
    while ($row = mysqli_fetch_array($result))
    {
