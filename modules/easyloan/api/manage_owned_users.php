...
function manage_owned_users(){
  ...
  $query = "SELECT act_mny_owned, act_mny_fine, act_mny_usr_id, act_info_nick, act_info_name, lns_app_id, lns_title, lns_category, lns_amount, lns_interest, lns_interest_rate, lns_repayment_method, lns_duration, lns_start, lns_end FROM account_money_act_mny LEFT JOIN account_info_act_info ON act_mny_usr_id = act_info_usr_id LEFT JOIN loans_lns ON act_mny_usr_id = lns_usr_id WHERE act_mny_is_owned = 1 AND lns_is_done = 0 ORDER BY act_mny_owned DESC LIMIT ".strval($start).",".strval($per_page);
  $result = mysqli_query($con, $query);
